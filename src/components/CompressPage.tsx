...
import { Upload, Download, Image, Settings, Trash2, Eye, Info, Zap, CheckCircle, AlertCircle, ToggleLeft, ToggleRight } from "lucide-react";
...
export const CompressPage: React.FC = () => {
  ...
  const compressImage = async (file: File, quality: number, format: ImageFileTypes, shouldResize: boolean, maxW?: number, maxH?: number): Promise<Blob> => {
  ...
  const compressFiles = async () => {
    ...
    for (let i = 0; i < pendingFiles.length; i++) {
      ...
      try {
        ...
        const format = enableCustomFormat ? outputFormat : getOriginalFormat(file.originalFile);
        ...
        const compressionRatio = ((file.originalSize - compressedBlob.size) / file.originalSize) * 100;
  ...
  const downloadFile = (file: CompressedFile) => {
    ...
    a.download = `compressed_${file.originalFile.name.split('.')[0]}.${extension}`;
  ...
  const downloadAllFiles = () => {
    files.filter(f => f.compressedBlob).forEach(downloadFile);
  };
              ...
              <ScrollArea className="h-[calc(100vh-250px)]">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <Label className="text-sm font-medium">Custom Output Format</Label>
                    <Button
                      variant="ghost"
                      size="sm"
                  ...
                  <p className="text-xs text-muted-foreground">
                    When disabled, images will keep their original format (JPG → JPG, PNG → PNG, etc.)
                  </p>
                </div>
                ...
                {enableCustomFormat && (
                  <div className="space-y-3">
                    ...
                    <Select value={outputFormat} onValueChange={(value) => setOutputFormat(value as ImageFileTypes)}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <p className="text-xs text-muted-foreground">
                      WebP provides the best compression with quality retention
                    </p>
                  </div>
                )}
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    When disabled, images will be compressed at their original dimensions
                  </p>
                </div>
              ...
              <CardHeader className="pb-4 flex-shrink-0">
                <div className="flex items-center justify-between">
                  <CardTitle className="flex items-center gap-2">
                    ...
                    {files.length === 0 ? 'Drop Images or Click to Add' : `Files (${files.length})`}
                  </CardTitle>
                  <div className="flex gap-2">
                    ...
                    <Button 
                      ...
                      variant="default"
                      disabled={isCompressing || files.length === 0 || files.every(f => f.status !== 'pending')}
                      className="flex items-center gap-2"
                      size="sm"
                    >
                      {isCompressing ? (
                        ...
                        <>
                          ...
                          Compress {files.filter(f => f.status === 'pending').length} Images
                        </>
                      )}
                    </Button>
                    {files.length > 0 && (
                ...
                {files.length === 0 ? (
                  ...
                  >
                    <div className="text-muted-foreground py-12">
                      <Upload className={`h-16 w-16 mx-auto mb-6 transition-colors ${isDragging ? 'text-primary' : ''}`} />
                      <h3 className="text-xl font-semibold mb-3">
                        {isDragging ? 'Drop your images here!' : 'Drop images here or click to select'}
                      </h3>
                      <p className="text-sm mb-4">
                        Supports JPEG, PNG, WebP, GIF, TIFF and other image formats
                      </p>
                    ...
                    {isDragging && files.length > 0 && (
                      <div className="absolute inset-0 bg-primary/10 border-2 border-primary border-dashed rounded-lg flex items-center justify-center pointer-events-none">
                        <div className="text-center text-primary">
                          ...
                          <p className="font-semibold">Drop to add more images</p>
                        </div>
                      </div>
                    )}
                  </div>
